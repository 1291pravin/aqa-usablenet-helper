...
//
// Usage: node scripts/aqa.mjs <subcommand> [--flag value ...]
...
async function req(method, path, body, { form = false } = {}) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    const { statusCode, body: resBody } = await undiciRequest(url, { method, headers, body: payload });
    const text = await resBody.text();
    const parsed = text ? safeJson(text) : undefined;
    if (statusCode >= 200 && statusCode < 300) return parsed;
    if (statusCode === 429 || (statusCode >= 500 && statusCode < 600)) {
      lastErr = httpErr(method, path, statusCode, parsed ?? text);
      await sleep(500 * Math.pow(2, attempt));
      continue;
    }
    throw httpErr(method, path, statusCode, parsed ?? text);
  }
  throw lastErr;
}
